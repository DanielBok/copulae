Copulae
=======

This section contains a simple overview of what a copula is along with the API of the various copulas available in
the package. For usage guides, check out the examples section.

.. toctree::
    :maxdepth: 2

    Introduction <introduction>
    Archimedean Copulas <archimedean/index>
    Elliptical Copulas <elliptical/index>
    Empirical Copula <empirical/index>
    Independent Copula <independent/index>
    Marginal Copula <marginal/index>
    Mixture Copula <mixtures/index>
